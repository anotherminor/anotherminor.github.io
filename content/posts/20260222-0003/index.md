...
![anotherminor 로고](/images/logo.svg)
이 문장은 이미지와 같은 문단 안에 있으므로 캡션처럼 보여야 한다.
...
긴 부연 설명이나 스포일러처럼 처음부터 모두 보일 필요가 없는 내용은 HTML의 `details`와 `summary` 태그로 접어둘 수 있다. 아래 예시는 제목 줄만 먼저 보이고, 사용자가 직접 눌렀을 때 숨겨둔 문단이 펼쳐지는지 확인하기 위한 용도다.
