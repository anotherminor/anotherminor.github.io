#!/usr/bin/env ruby
#
# validate-content.rb — 포스트 프론트매터 유효성 검사
#
# 용도:
#   content/posts/ 하위 모든 index.md 파일의 YAML 프론트매터를 읽어
#   필수 필드 존재 여부, 타입, 허용 값 등을 검사한다.
#   오류가 하나라도 있으면 exit 1로 종료하므로 CI/CD 게이트로 사용 가능하다.
#
# 검사 항목:
#   - 필수 필드: title, date, draft, slug, categories
...
#   - formats: [link]이면 external_url이 유효한 http/https URL이어야 함
#   - tags: 제공된 경우 배열이어야 함
#   - summary: 제공된 경우 문자열이어야 함
...
#
# CI에서 자동 실행:
#   .github/workflows/deploy-github-pages.yml 의 prebuild 단계에서 실행됨
