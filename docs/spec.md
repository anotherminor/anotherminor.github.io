...
같은 포인트 크기라도 서체마다 실제 시각적 높이가 다르므로, 행간은 포인트 수치 자체보다 읽을 때 보이는 글자 높이를 기준으로 잡는다. 이 블로그는 WCAG의 최소 `1.5`를 목표값이 아니라 하한선에 가까운 기준으로 해석하며, 한글 읽기 텍스트는 대체로 `1.45`에서 `1.75` 사이에서 운용한다. 상세 본문 `.prose-body`는 이 범위 안에서 `1.68`을 기본값으로 유지하고, 리드 문장·요약·검색 발췌·캡션 같은 읽기 계열 텍스트는 역할에 따라 `--reading-line-height-relaxed` 또는 `--reading-line-height-support`를 사용한다.

읽기 계열 텍스트의 행간은 개별 셀렉터에 숫자를 직접 복사하지 않고 읽기 전용 변수에서 선택한다. 반면 네비게이션, 메타, 배지, 입력창처럼 읽기 본문이 아닌 UI 크롬은 이 체계에 편입하지 않는다.
...
이미지 캡션 여부는 별도 문법이 아니라 마크다운의 빈 줄 규칙으로 결정한다. 전제 조건은 `hugo.yaml`에 `markup.goldmark.renderHooks.image.useEmbedded: always`가 설정돼 이미지를 `<figure>`가 아닌 `<img>` 인라인으로 렌더링하는 상태다.
