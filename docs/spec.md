...
작성자는 이미지 바로 다음 줄에 텍스트를 쓰면 캡션으로 처리된다는 점을 이해해야 하며, 일반 문단을 이어 쓰려면 반드시 빈 줄을 넣는다. 이 캡션 규칙 자체는 마크다운 클래스 추가나 HTML 구조 변경에 기대지 않는다. 다만 본문 이미지의 `alt` 텍스트를 데스크톱 호버와 터치 기기 길게 누르기에서 임시 툴팁으로 보여 주는 JavaScript는 별도로 허용한다.
